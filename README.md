...
- Requires every newly authorised clean artifact to have been created no later than the originating trusted live marker's persisted `maxWaitDeadlineAt`, or its legacy deadline derived from the live marker's `headStartedAt`/`createdAt` and the current maximum wait; persisted live wait-budget fields must match sticky state exactly, while an already-recorded legacy state deadline may only narrow the derived window. Reconciliation may finish after the deadline, but an artifact first created after it cannot pass or reassert the gate.
